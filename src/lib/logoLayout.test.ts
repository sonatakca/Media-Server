...
describe("reading a stored layout", () => {
  ...
  it("rejects a partial or non-numeric layout rather than half-applying it", () => {
    ...
    expect(getLogoLayout(item({ x: "0.5", y: 0.5, width: 0.5 }))).toBeNull();
    expect(getLogoLayout(item({ x: Number.NaN, y: 0.5, width: 0.5 }))).toBeNull();
  });

  it("clamps a stored layout that is out of range", () => {
    expect(getLogoLayout(item({ x: 2, y: -1, width: 5 }))).toEqual({
      x: 1,
      y: 0,
...
describe("layout geometry", () => {
  it("anchors by centre so a drag tracks the pointer", () => {
    expect(getLogoLayoutStyle({ x: 0.25, y: 0.4, width: 0.6 })).toEqual({
      left: "25%",
      top: "40%",
      width: "60%",
  ...
  it("keeps a logo legible and smaller than the card", () => {
    expect(clampLogoLayout({ x: 0.5, y: 0.5, width: 0.01 }).width).toBe(
      MIN_LOGO_WIDTH,
    );
    expect(clampLogoLayout({ x: 0.5, y: 0.5, width: 9 }).width).toBe(
      MAX_LOGO_WIDTH,
    );
  });
  ...
  it("falls back to the opening size when the width is not a number", () => {
    expect(
      clampLogoLayout({ x: 0.5, y: 0.5, width: Number.NaN }).width,
...
describe("dragging", () => {
  ...
  it("stops at the edges of the card", () => {
    const moved = moveLogoLayout({ x: 0.9, y: 0.1, width: 0.5 }, 400, -400, CARD);
    expect(moved).toMatchObject({ x: 1, y: 0 });
  });

  it("does nothing before the card has been measured", () => {
    const layout = { x: 0.5, y: 0.5, width: 0.5 };
