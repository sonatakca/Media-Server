...
function getStreamLabel(stream: JellyfinMediaStream, fallback: string, t: (key: TranslationKey) => string): string {
  const parts = [
    stream.DisplayTitle,
    stream.Title,
    stream.Language?.toUpperCase(),
    stream.Codec?.toUpperCase(),
    stream.Channels ? t("details.audioChannelsShort").replace("{count}", String(stream.Channels)) : undefined,
  ...
  return (
    <div className="fixed inset-x-2 bottom-[calc(env(safe-area-inset-bottom)+0.5rem)] z-[70] max-h-[calc(100dvh-1rem)] overflow-hidden rounded-2xl border border-white/10 bg-[rgba(18,18,20,0.96)] shadow-[0_24px_90px_rgba(0,0,0,0.72)] backdrop-blur-2xl sm:absolute sm:inset-x-auto sm:bottom-[4.25rem] sm:right-0 sm:w-[min(22rem,calc(100vw-2rem))]">
      <div className="border-b border-white/10 px-4 py-3">
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--accent)]">{t("settings.settings")}</p>
        <h2 className="mt-0.5 text-base font-black text-white">{t("settings.playbackOptions")}</h2>
      </div>
      ...
      <div className="max-h-[calc(100dvh-5.25rem)] overflow-y-auto p-2 sm:max-h-[min(28rem,calc(100svh-12rem))]">
        ...
        <SettingsButton
          ...
          subtitle={selectedQualityId === "auto" ? t("settings.bestJellyfinSource") : t("settings.useBestJellyfinSource")}
        ...
        {qualityOptions.length > 0 ? (
          ...
          <SettingsButton title={t("settings.manualQuality")} subtitle={t("settings.noAlternateQualities")} disabled hasSubmenu />
        )}
        ...
        {audioStreams.length > 0 ? (
          audioStreams.map((stream, index) => (
            <SettingsButton
              ...
              title={getStreamLabel(stream, formatTemplate(t("settings.audioTrack"), { number: index + 1 }), t)}
              subtitle={
        ...
        <SettingsButton
          ...
          subtitle={selectedSubtitleStreamIndex === -1 ? t("settings.subtitlesOff") : t("settings.disableSubtitles")}
        ...
        {subtitleStreams.length > 0 ? (
          subtitleStreams.map((stream, index) => (
            <SettingsButton
              ...
              title={getStreamLabel(stream, formatTemplate(t("settings.subtitle"), { number: index + 1 }), t)}
              subtitle={
