 ...
 *   trickplay/sprite_0.jpg   seek-bar preview sheets
 * ```
 *
...
/**
 * Seek-bar preview sheets, generated by Seyirlik from the title's own source.
 *
 * A title's sheets belong to the title for the same reason its rungs do: they
 * are derived from one source, they are meaningless beside any other, and a
 * library that can be moved or backed up without the server has to carry them
 * along with everything else it owns. They were previously written into a
 * central tree keyed by a UUID, which made them the one generated asset a
 * person could not find by opening the folder.
 *
 * Deliberately the bare word. The `*.trickplay` folders a Jellyfin-era library
 * is full of are a *different* population — external, historical, and never
 * live — and the naming is what keeps the two apart: `trickplay` is the
 * managed directory, `<something>.trickplay` never is.
 */
export const TITLE_TRICKPLAY_DIRECTORY = "trickplay";
