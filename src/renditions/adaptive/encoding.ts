...
/** One `-i` and the input options that belong to it. */
export interface AdaptiveEncodingInput {
  path: string;
  /** Demuxer to force, e.g. `lavfi`. */
  format?: string;
  /** Accurate input seek, in seconds. */
  startSeconds?: number;
  /** How much of this input to read, in seconds. */
...
export interface AdaptiveAudioOutput {
  ...
  /**
   * Which stream of *this invocation's input* carries the track, when it is not
   * the source stream index.
   *
   * The two are the same for every ordinary encode, and differ only when a
   * track is being packaged from something that is not the original file — a
   * salvaged track assembled from per-range intermediates, whose only stream is
   * zero. The rendition is still named after the source stream, because that is
   * the identity a saved audio preference points at.
   */
  mapStreamIndex?: number;
  /**
   * A filter-graph output to map instead of an input stream, e.g. `[a0]`.
   *
   * Used when a track is assembled rather than read: a salvaged soundtrack is
   * the readable stretches of the source and generated silence for the rest,
   * concatenated inside one filter graph so the whole track is encoded in a
   * single continuous pass. Encoding the pieces separately and joining the
   * files afterwards would add every part's encoder priming to the timeline and
   * walk the sound out of step with the picture.
   */
  mapLabel?: string;
...
export interface AdaptivePackageEncodingInput {
  inputPath: string;
  /**
   * Demuxer to force for the input, e.g. `lavfi`.
   *
   * Present only for synthetic input: black video standing in for an unreadable
   * stretch of a damaged source is generated by `color`, and it has to travel
   * through this same builder so the replacement is encoded with byte-for-byte
   * the same settings as the epochs around it. That is what keeps every
   * rendition's initialisation segment identical and the epochs joinable.
   */
  inputFormat?: string;
  /**
   * Several inputs, when one is not enough.
   *
   * Present only for a salvaged audio track, which reads the source once per
   * undamaged stretch and generates silence for the rest. Takes the place of
   * `inputPath`, `inputFormat` and `startSeconds`; everything else about the
   * command is unchanged, which is the point — the output side must not fork.
   */
  inputs?: readonly AdaptiveEncodingInput[];
  /**
   * A filter graph for the audio side, joined to the video one.
   *
   * The audio outputs that consume it name their pads with `mapLabel`.
   */
...
export function buildAdaptivePackageFfmpegArgs({
  ...
  const inputList: readonly AdaptiveEncodingInput[] =
    inputs && inputs.length > 0
      ? inputs
      : [
          {
            path: inputPath,
            ...(inputFormat === undefined ? {} : { format: inputFormat }),
            ...(startSeconds === undefined ? {} : { startSeconds }),
          },
        ];
  const primaryStart = inputList[0]?.startSeconds;
  const filterGraph = [
    videoOutputs.length > 0
      ? buildAdaptiveFilterComplex({
          ...
          encoder,
          ...(hdr ? { hdr } : {}),
          trimSeekPreroll: primaryStart !== undefined && primaryStart > 0,
        })
      : "",
    audioFilterComplex ?? "",
  ]
  ...
  const args = [
    ...
    ...(videoOutputs.length > 0 && filterComplexThreads !== undefined
      ...
      : []),
    ...inputList.flatMap((input) => [
      // Before `-i`, so it seeks the input rather than discarding decoded
      // output; `-t` beside it limits how much of the input is read.
      ...(input.startSeconds === undefined || input.startSeconds <= 0
        ? []
        : ["-ss", input.startSeconds.toFixed(6)]),
      ...(input.durationSeconds === undefined || input.durationSeconds <= 0
        ? []
        : ["-t", input.durationSeconds.toFixed(6)]),
      ...(input.format === undefined ? [] : ["-f", input.format]),
      "-i",
      input.path,
    ]),
    // A run with nothing to filter has no graph, and an empty filter graph is
    // not a valid argument, so the flag itself is omitted rather than passed
    // with nothing in it.
    ...(filterGraph.length > 0 ? ["-filter_complex", filterGraph] : []),
  ];
  ...
  audioOutputs.forEach((output) => {
    args.push(
      "-map",
      output.mapLabel ??
        `0:${output.mapStreamIndex ?? output.sourceStreamIndex}`,
    );
  });
