...
import {
  ...
  getDefaultSubtitlePreferenceIndex,
  getDetailValue,
...
/**
 * The Metadata Edit half of Library Maintenance.
 *
 * Lifted out of the page whole rather than rewritten: this is the editor that
 * was already here — the same search, the same filters, the same per-type
 * fields, artwork and subtitle defaults — and the change is that it is now an
 * explicit mode of its own instead of a column beside the scan controls.
 *
 * It keeps its own "refresh this library's metadata" action, because that
 * action is driven by the library filter standing directly above it. Moving it
 * to the scan tab would have meant a second copy of the filter, which is the
...
export function MetadataEditPanel() {
  const { t } = useLanguage();
  ...
  return (
    <div className="space-y-5">
      {scanState.message ? (
        <p
          role="status"
          className={`relative rounded-2xl border px-4 py-3 text-sm font-bold ${
