...
interface EpubContentView {
  document: Document;
  addClass(className: string): void;
  addStylesheetCss(css: string, key: string): unknown;
}

const READER_SETTINGS_KEY = "seyirlik.reader.settings";
const READER_PROGRESS_KEY = "seyirlik.reader.progress";
...
const FONT_SCALE_STEPS = Array.from(
  { length: 14 },
  (_, index) => 80 + index * 5,
);
const LINE_HEIGHT_STEPS = Array.from({ length: 14 }, (_, index) =>
  Number((1.25 + ((2.2 - 1.25) * index) / 13).toFixed(2)),
);
const WIDTH_STEPS = Array.from({ length: 13 }, (_, index) => 48 + index * 4);

const EPUB_PREPARATION_TIMEOUT_MS = 15000;

const EPUB_REVEAL_STYLES = `
...
const FORMAT_EXTENSIONS: Record<Exclude<ReaderFormat, "fallback">, string[]> = {
  epub: ["epub"],
  pdf: ["pdf"],
...
type ReaderPalette = (typeof themePalettes)[ReaderTheme];

function getReaderControlStyle(
...
function getThemePreviewControlStyle(
  palette: ReaderPalette,
  active: boolean,
  flat = false,
): CSSProperties {
...
function readStoredReaderSettings(): ReaderSettings {
...
function readReaderProgress(itemId: string): StoredReaderProgress | null {
...
function getNormalizedExtension(value?: string): string | null {
  if (!value) {
    return null;
  }
...
function getReaderFormat(item: JellyfinItem): ReaderFormat {
  const candidates = [
...
function getFormatLabel(format: ReaderFormat): string {
  if (format === "fallback") {
    return "FILE";
  }
